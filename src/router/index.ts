...
router.beforeEach(async (to: RouteLocationNormalized, from: RouteLocationNormalized, next) => {
  ...
  if (to.name === 'assessment-detail') {
    ...
    logger.log('🛡️ Router Guard: Checking assessment-detail access for ID:', assessmentId)
    
    const appStore = useAppStore()
    const hasAssessment = appStore.hasAssessment(assessmentId)
    
    logger.log('🛡️ Router Guard: hasAssessment result:', hasAssessment)
    logger.log('🛡️ Router Guard: Current assessment in store:', appStore.currentAssessment)

    if (!hasAssessment) {
      logger.log('🛡️ Router Guard: Assessment not found, redirecting to /assessment')
      // 如果测评不存在，重定向到测评开始页面
      next({ name: 'assessment' })
      return
    } else {
      logger.log('🛡️ Router Guard: Assessment found, allowing access')
    }
  }
...
router.afterEach((to: RouteLocationNormalized) => {
  ...
  // 可以在这里添加页面访问统计等逻辑
  logger.log(`Navigated to: ${to.path}`)
})
